...
def read_whole_dump(file: str) -> list[dict[str:any]]:
    ...
    loopCount = 0 #intializes the frame loop counter

    with open(file, "r") as myfile:
        for line in myfile:
            loopCount += 1
            if "TIMESTEP" in line: #how we know we are in a new frame
                loopCount = 0 #keeps track of how many loops we have gone through in a frame
                mode = "timeStep" #resets the lineCount every frame
                checkLoop = loopCount
            ...
            if mode == 'timeStep' and (loopCount - checkLoop) == 1: #goes to the line after the one with the timestep title
            ...
            if mode == "atomNum" and (loopCount - checkLoop) == 1: # grabs the line with the Number of atoms information
            ...
            if mode == "bounds" and (loopCount - checkLoop) == 2: # lines that contain values of BOX BOUNDS, will be organized depending on boundary type
            ...
            if "id" in line: #line that contains information about the atom information being dumped, generates format of output dictionary 
                mode = "atom"
                checkLoop = loopCount
                v_line = line.split() #store all the parameters in a list
            if mode == "atom" and (loopCount - checkLoop) >= 1: #lines 10 and up contain dump data of each atom
